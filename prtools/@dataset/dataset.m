...
function varargout = dataset(a,labels,varargin)
	...
	if isempty(PRTOOLS_START)
		...
		PRTOOLS_START = 1;
		if isdeployed
			x = datafile; % force loading of datafile
			x = mapping;  % and mapping libraries
		else
		it_was = 734787; % ## expiring ##
% 			it__is = datenum(date);
% 			nexpd = 25 + 6 - it__is + it_was;
			z = prtver; z = z{1};
			disp([newline '    Welcome to PRTools, version ' z.Version ...
                       ' (' z.Date ').'])
			disp('    This version of PRTools is valid for non-commercial use only.')
